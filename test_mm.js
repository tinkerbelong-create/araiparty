...
/* 指定のフェーズ・本編まで進める */
const toPhase = (G, id) => { let n=0; while ((G.phase.id !== id || G.step !== 'main') && n++<40) allReady(G); return G; };
const goto = (G, id) => { G.phaseIdx = G.sc.phases.findIndex(p=>p.id===id); G.step='main'; return G; };
...
{
  const G = g();
  T('★最初は「はじめに」(マダミスの説明)から', () => {
    assert.strictEqual(G.phase.id, 'intro');
...
}
{
  ...
  T('★マダミスの説明が用意されている', () => {
    const s = MM.loadScenario('cake');
    assert(s.intro && /マーダーミステリー/.test(s.intro) && s.intro.length > 100);
  });
}
